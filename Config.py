...
def __init_settings():
    ...
    settings = {'bangumi_dir': '',
                ...
                'multi_downloading_segment': 2,  # 在上面配置為 True 時有效, 每個影片並發下載分段數
...
def __update_settings(old_settings):  # 升級配置檔案
    ...
    if 'multi_downloading_segment' not in new_settings.keys():  # v3.1 新增分段下載模式下每個影片並發下載分段數
    ...
    if 'use_copyfile_method' not in new_settings.keys():
        # v6.0 新增影片轉移方法開關, 配置 True 以適配 rclone 掛載盤
...
def read_settings(config=''):
    ...
    if 'database_version' in settings.keys():
        ...
        # 如果該版本配置下沒有 database_version 項, 則資料庫版本應該是1.0
    ...
    if not (re.match(r'^http://', settings['proxy'].lower())
            ...
            or re.match(r'^socks5://', settings['proxy'].lower())  # v12開始原生支援 socks5 代理
            or re.match(r'^socks5h://', settings['proxy'].lower())):  # socks5h 遠端解析域名
        #  如果出現非自身支援的協議
        use_gost = True  # 則啟用gost
...
def read_latest_version_on_github():
    ...
    try:
        ...
        remote_version['tag_name'] = aniGamerPlus_version  # 拉取github版本號失敗
        remote_version['body'] = ''
