...
def insert_db(anime):
    ...
    try:
        ...
        err_print(anime_dict['sn'], 'ＤＢ錯誤', 'title=' + anime_dict['title'] + ' 資料已存在！' + str(e), status=1)
...
def __kill_thread_when_ctrl_c():
    # 等待所有任務完成
    for t in thread_tasks:  # 當使用者 Ctrl+C 可以 kill 執行緒
        while True:
            if t.is_alive():
                time.sleep(1)
            else:
...
def __init_proxy():
    if settings['use_gost']:
        ...
        print('使用代理連線動畫瘋, 使用http/https/socks5協議')
...
def run_dashboard():
    ...
    if not port_is_available(settings['dashboard']['port']):
        err_print(0, 'Web控制面板啟動失敗', 'Port已被佔用! 請到配置檔案更換', status=1, no_sn=True)
        return
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:  # 支援命令列使用
        ...
        if arg.current_path:
            ...
            info = '使用命令列模式, 檔案將儲存在配置檔案中指定的目錄下: '
