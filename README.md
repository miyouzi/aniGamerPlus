...
windows 使用者可以[**點選這裡**](https://github.com/miyouzi/aniGamerPlus/releases/latest)下載exe檔案使用.
...
**config-sample.json**為範例配置檔案, 可以將其修改後改名為**config.json**.

若不存在**config.json**, 則程式在執行時將會使用預設配置建立.

```jsonc
{
    "bangumi_dir": "",  // 下載存放目錄, 動畫將會以番劇為單位分資料夾存放
    "temp_dir": "",  // 臨時目錄位置, v9.0 開始下載中檔案將會放在這裡, 完成後再轉移至番劇目錄, 留空預設在程式所在目錄的 temp 資料夾下
    ...
    "multi_downloading_segment": 3,  // 每個影片最大並發下載分段數, 僅在 "segment_download_mode" 為 true 時有效, 最高為 5, 超過將重置為 5
    "add_bangumi_name_to_video_filename": true,  // 如果為 false, 則只有劇集名, 若劇集名為個位數字, 則補零
    "add_resolution_to_video_filename": true,  // 是否在影片檔名中新增清晰度, 格式舉例: [1080P]
    "customized_video_filename_prefix": "【動畫瘋】",  // 影片檔名字首
    "customized_bangumi_name_suffix": "",  // 影片檔名中番劇名的字尾, 在劇集名之前
    ...
    "dashboard": {  // Web控制面板配置
        ...
        "BasicAuth": false,  // 是否使用 BasicAuth 進行認證, 注意, 使用者密碼是明文傳輸的, 如有需要建議同時啟用 SSL
        "username": "admin",  // BasicAuth 使用者名稱
        "password": "admin"  // BasicAuth 密碼
    },
    "save_logs": true,  // 是否記錄日誌, 一天一個日誌
    "quantity_of_logs": 7,  // 日誌保留數量, 正整數值, 必須大於等於 1, 預設為 7
    "config_version": 14.0,  // 配置檔案版本
...
自v6.0開始, 新增對番劇進行分類功能, 在一排番劇列表的上方 **@** 開頭後面的字元將會作為番劇的分類名, 番劇會歸類在此分類名的資料夾下
...
範例:
```
11415 <魔法少女特殊戰明日香> # 魔法少女特殊戰明日香
11433 <えんどろ～！> # ENDRO！
11354 latest <刀劍神域3> # 刀劍神域 Alicization
11398 # 粉彩回憶
```

### aniGamer.db

sqlite3資料庫, 可以使用 [SQLite Expert](http://www.sqliteexpert.com/) 等工具開啟編輯

記錄影片下載狀態等相關資訊, 一般無需改動

欄位設計:
- ```sn``` sn值 (PK)
- ```title``` 完整標題
- ```anime_name``` 番劇名
- ```episode``` 劇集名, 一般為數字, 也可能是```特別篇``` ```電影``` 等
...
支援命令列使用, 檔案預設將儲存在**config.json**中指定的目錄下

**配置檔案中的代理配置同樣適用於命令列模式!**

**除了使用 list 模式的情況, 命令列模式將不會和資料庫進行互動, 將會無視資料庫中下載狀態標記強制下載**
...
```
 ...
 - **-m** 接下載模式, 可空, 空則下載傳入sn碼的影片

    - **single** 下載此 sn 單集(預設)

    - **multi** 下載多個sn, 啟用此模式時, 透過```-e```傳入多個sn, sn之間使用英文```,```分割
 ...
 - **-t** 接最大並發下載數, 可空, 空則讀取**config.json**中的定義
